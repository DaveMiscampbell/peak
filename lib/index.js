...
Peak.prototype = {
  ...
  writeHtml: function() {
    var compiler = new Compiler(this, this.config.options('deploy') || {});

    return compiler.compile_file(this.config.options('deploy').index).then((compiledFile) => {
      fs.writeFileSync(this.config.options('writeHtml').outputPath, compiledFile);
    });
  },
